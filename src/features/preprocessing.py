...
def gen_data(base_image, quantity, label):
    ...
    for i in range(0, quantity):
        ...
        img = random_noise(img, mode='pepper', seed=b, clip=True)
		
		# randomly rotate image by a maximum of 10 degrees
        img = transform.rotate(img, a, resize=False, center=None, order=1, mode='constant', cval=1,
...
# carrega as imagens presentes em determinado path
def load_images(path):
    images = []
    labels = []
    dictionary = {}
	
	# percorre os subdiretórios e carrega as imagens presentes neles
    for k, d in enumerate(listdir(path)):
...
# exibe as imagens presentes no array de entrada, em uma matriz com linhas = rows e colunas = columns
...
# gera os feature descriptors e visualização dos HOGS
def hog_images(imgs):
	# lista de descriptors e visualizações HOG
    fds, hog_imgs = [], []
    for i, image in enumerate(imgs):
		# gera os HOGs com a seguinte configuração, que gera as imagens rapidamente sem interferir na sua qualidade e por consequência, nos classificadores
...
# se a constante PLOT estiver com valor True, exibe as imagens
if PLOT:
...
fd_icmc, hog_icmc = hog_images(dataset_icmc)
if PLOT:
...
fd_orl, hog_orl = hog_images(dataset_orl)
if PLOT:
    plot_images(hog_orl, math.ceil(len(hog_orl) / 4), 4)
print('HOG generated')

# cria os diretórios, caso eles não existam para salvar as imagens
...
np.save(pre_icmc_hogs + '/fd', np.array(fd_icmc))
np.save(pre_icmc_hogs + '/labels', np.array(labels_icmc))
print('HOG saved')

print('Saving orl HOG')
np.save(pre_orl_hogs + '/fd', np.array(fd_orl))
